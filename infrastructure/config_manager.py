...
# -*- coding: utf-8 -*-
"""
Config Manager - EAS Híbrido 2025 Unified Configuration
...
try:
    ...
    logger = get_eas_logger("EAS.ConfigManager")
except ImportError:
    # Fallback a logger estándar
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger("EAS.ConfigManager")
...
try:
    ...
    logger.warning("⚠️ Watchdog no disponible - hot-reload desactivado")
...
try:
    ...
    logger.warning("⚠️ jsonschema no disponible - validación desactivada")
...
if WATCHDOG_AVAILABLE:
    class ConfigChangeHandler(FileSystemEventHandler):
        """Handler optimizado para hot-reload en producción"""
        
        def __init__(self, config_manager: 'ConfigManager'):
            self.config_manager = config_manager
            self.debounce_timers: Dict[str, threading.Timer] = {}
            self.debounce_delay = 2.0  # 2 segundos para evitar múltiples reloads
        ...
        def _handle_config_change(self, file_path: str, config_file: str):
            """Ejecuta reload después del período de debounce"""
            try:
                logger.info(f"🔄 Config change detected: {config_file}")
...
class ConfigManager:
    """
    Gestor de configuración unificada EAS Híbrido 2025
    Soporte para configuración jerárquica con migración desde legacy
    """
    ...
    def __init__(self, config_base_path: Path, enable_hot_reload: bool = True):
        ...
        self.observer = Observer() if self.enable_hot_reload else None
        self.change_handler = ConfigChangeHandler(self) if self.enable_hot_reload else None
    ...
    def _build_legacy_mapping(self) -> Dict[str, Tuple[ConfigType, str]]:
        ...
        return {
            ...
            # hecta_gold_config.json → trading_config.json  
            ...
            # hecta_gold_config.json → agents_config.json
    ...
    async def initialize(self):
        """Inicialización asíncrona completa"""
        try:
            # 1. Cargar configuraciones principales
            await self._load_all_configs()
            
            # 2. Cargar configuraciones legacy para migración
            await self._load_legacy_configs()
            
            # 3. Inicializar esquemas de validación
            await self._initialize_validation_schemas()
            
            # 4. Iniciar hot-reload si está habilitado
            if self.enable_hot_reload and self.observer:
                ...
                self.observer.start()
                logger.info("👀 Hot-reload habilitado para configuración")
            ...
            logger.info("✅ ConfigManager inicializado exitosamente")
    ...
    async def _load_single_config(self, config_type: ConfigType, file_path: Path):
        """Carga una configuración individual"""
        try:
            if file_path.exists():
                ...
                logger.info(f"✅ Config loaded: {config_type.value}")
                
            else:
                logger.warning(f"⚠️ Config file not found: {file_path}")
        ...
        for name, filename in legacy_files.items():
            ...
            if file_path.exists():
                try:
                    ...
                    logger.info(f"📁 Legacy config loaded: {name}")
                except Exception as e:
                    logger.warning(f"⚠️ Error loading legacy config {name}: {e}")
            else:
                logger.debug(f"📁 Legacy config not found: {name}")

    async def _initialize_validation_schemas(self):
        """Inicializa esquemas de validación JSON"""
        # Esquema básico para trading config
    ...
    async def _validate_all_configs(self) -> bool:
        ...
        if not JSONSCHEMA_AVAILABLE:
            logger.warning("⚠️ Validación desactivada - jsonschema no disponible")
            return True
        ...
        for config_type, config_data in self.configs.items():
            if config_type in self.validation_schemas:
                try:
                    ...
                    logger.debug(f"✅ Config validation passed: {config_type.value}")
                except ValidationError as e:
                    logger.error(f"❌ Config validation failed for {config_type.value}: {e}")
                    if PROMETHEUS_AVAILABLE:
    ...
    def get_config(self, key_path: str, use_legacy_fallback: bool = True) -> Any:
        """
        Obtiene valor de configuración con fallback a legacy
        ...
        with self.config_lock:
            try:
                ...
                # FALLBACK A LEGACY SI ESTÁ HABILITADO
                if use_legacy_fallback and self.legacy_configs:
                    legacy_value = self._get_from_legacy(key_path)
                    if legacy_value is not None:
                        logger.debug(f"🔄 Using legacy value for: {key_path}")
                        return legacy_value
                
                logger.warning(f"⚠️ Config value not found: {key_path}")
                return None
                
            except Exception as e:
                logger.error(f"❌ Error getting config {key_path}: {e}")
                return None

    def _get_from_legacy(self, key_path: str) -> Any:
        """Intenta obtener valor desde configuraciones legacy"""
        # Buscar en mapeo legacy
        for (legacy_key_parts, (config_type, new_path)) in self.legacy_mapping.items():
            if key_path == new_path:
                legacy_file = legacy_key_parts[0]
                legacy_config = self.legacy_configs.get(legacy_file, {})
                
                # Manejar claves anidadas
                if len(legacy_key_parts) == 2:
                    legacy_key = legacy_key_parts[1]
                    if isinstance(legacy_key, str):
                        return legacy_config.get(legacy_key)
                    elif isinstance(legacy_key, tuple):
                        current = legacy_config
                        for key_part in legacy_key:
                            if isinstance(current, dict) and key_part in current:
                                ...
                                break
                        return current
                elif len(legacy_key_parts) == 3:
                    section, subsection, key = legacy_key_parts[1:]
                    if section in legacy_config and subsection in legacy_config[section] and key in legacy_config[section][subsection]:
                        return legacy_config[section][subsection][key]
        
        return None

    def set_config(self, key_path: str, value: Any) -> bool:
        """
        Establece valor de configuración (para testing/dynamic adjustment)
    ...
    def reload_config_file(self, file_path: str) -> bool:
        """Recarga archivo de configuración específico"""
        ...
        try:
            with self.config_lock:
                ...
                # Determinar tipo de configuración basado en nombre de archivo
                config_type = None
                if config_file == "trading_config.json":
                    ...
                    logger.warning(f"⚠️ Unknown config file: {config_file}")
                    return False
                ...
                # Validar
                if JSONSCHEMA_AVAILABLE and config_type in self.validation_schemas:
                ...
                logger.info(f"🔄 Config reloaded: {config_file}")
                return True
            ...
            if PROMETHEUS_AVAILABLE:
                CONFIG_RELOADS.labels(status='error').inc()
            logger.error(f"❌ Config reload failed for {config_file}: {e}")
            return False
    ...
    async def save_config(self, config_type: ConfigType) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ Error saving config {config_type.value}: {e}")
            return False
    ...
    def force_reload_all(self) -> bool:
        ...
        logger.info("🔄 Forcing reload of all configs")
        try:
...
# Decorator para inyección de configuración
def with_config(config_path: str):
    """
    Decorator para inyectar valores de configuración en funciones
    
    Usage:
        @with_config("trading.core_trading.instrument")
        def some_function(..., config_value=None):
            # config_value contiene el valor de la configuración
    """
    def decorator(func):
